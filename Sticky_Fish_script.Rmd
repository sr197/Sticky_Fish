...
#this also takes about 20 minutes
...
#this is because they changed trawl doors to catch more pelagic species in 1985
...
specnames <- specnames[!(specnames %in% c("SYACIUM.PAPILLOSUM"))]
...
for (i in 1:length(specnames)) { #for each species in spec_list
  fit <- gam(as.formula(paste(specnames[i],"~ s(depth) + s(bottemp) + s(botsalin) + factor(SEDIMENT) + s(surfsalin) + s(botsalin)")), data=fall, method="REML")
#standardized <- lm.beta(fit)
        fit_depth <- gam(as.formula(paste(specnames[i],"~ s(depth)")), data=fall, method="REML")
          fit_bt <- gam(as.formula(paste(specnames[i],"~ s(bottemp)")), data=fall, method="REML")
           fit_ssal <- gam(as.formula(paste(specnames[i],"~ s(surfsalin)")), data=fall, method="REML")
         fit_sed <- gam(as.formula(paste(specnames[i],"~ factor(SEDIMENT)")), data=fall, method="REML")
         fit_bsal <- gam(as.formula(paste(specnames[i],"~ s(botsalin)")), data=fall, method="REML")
         fit_sst <- gam(as.formula(paste(specnames[i],"~ s(surftemp)")), data=fall, method="REML") 
...
for (i in 1:length(specnames)) { #for each species in spec_list
  fit <- gam(as.formula(paste(specnames[i],"~ s(depth) + s(bottemp) + s(botsalin) + factor(SEDIMENT) + s(surfsalin) + s(botsalin)")), data=spring, method="REML")
#standardized <- lm.beta(fit)
        fit_depth <- gam(as.formula(paste(specnames[i],"~ s(depth)")), data=spring, method="REML")
          fit_bt <- gam(as.formula(paste(specnames[i],"~ s(bottemp)")), data=spring, method="REML")
           fit_ssal <- gam(as.formula(paste(specnames[i],"~ s(surfsalin)")), data=spring, method="REML")
         fit_sed <- gam(as.formula(paste(specnames[i],"~ factor(SEDIMENT)")), data=spring, method="REML")
         fit_bsal <- gam(as.formula(paste(specnames[i],"~ s(botsalin)")), data=spring, method="REML")
         fit_sst <- gam(as.formula(paste(specnames[i],"~ s(surftemp)")), data=spring, method="REML") 
        dev.depth <- summary(fit_depth)$dev.expl
        dev.bt <- summary(fit_bt)$dev.expl
        dev.ssal <- summary(fit_ssal)$dev.expl
        dev.sst <- summary(fit_sst)$dev.expl
...
ocean_adapt <- neus_done_by_sarah
