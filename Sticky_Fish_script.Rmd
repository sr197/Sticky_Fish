...
##5.2 distance shifted first five years to last five years (1984 to 2018) 
...
###5.3.4 Clean kernel 
calculate changes in min and max lat/lon and percentage change in area (from above created datasets)
```{r}
...
#get out the most important variable 
...
ggplot(data=subset(fall_dev_dist_spec, !is.na(strong)), aes(x= strong,  group=depth_profile)) + 
  ...
  ggsave(filename= "Figure2.jpg", plot=last_plot(), width = 18, height=9, units=c("cm"), dpi=500)
...
ggplot(data=data.m.f.2, aes(x=depth_profile, y=value, fill=depth_profile)) + geom_boxplot() +
  stat_summary(fun.y=mean, colour="darkred", geom="point", 
               shape=18, size=3,show_guide = FALSE) +
 ...
 theme_classic(base_size=10) +
  ...
  ggsave(filename= "Figure3.jpg", plot=last_plot(), width = 18, height=18, units=c("cm"), dpi=500)
...
ggplot(data=data.m.s.2, aes(x=strong, y=value, fill=strong)) + geom_boxplot() +
 ...
 theme_classic(base_size=10) +
  theme(axis.title = element_text(size = rel(1.5)),
        ...
        strip.text = element_text(size = rel(1)), 
        panel.spacing = unit(1, "cm")) +
  theme(legend.position = "none") + scale_fill_simpsons() + scale_color_simpsons() + ggsave(filename= "Figuure1_spring.jpg", plot=last_plot(), width = 18, height=18, units=c("cm"), dpi=500)
...
ggplot(data=data.m.s.2, aes(x=depth_profile_reviewer, y=value, fill=depth_profile)) + geom_boxplot() +
  stat_summary(fun.y=mean, colour="darkred", geom="point", 
               shape=18, size=3,show_guide = FALSE) +
    labs(y = "", x = "Species type") + stat_compare_means(comparisons = my_comparisons, method="wilcox.test", label = "p.format", size=2.5) + labs(fill = "Species Type") + 
  facet_wrap(~variable, scales = "free") + 
 theme_classic(base_size=10) +
  theme(axis.title = element_text(size = rel(1.5)),
        ...
        panel.spacing = unit(1, "cm")) + 
  theme(legend.position = "none") + scale_fill_manual(values=c("#999999", "#E69F00", "#56B4E9")) + 
  ggsave(filename= "Figure3_spring.jpg", plot=last_plot(), width = 18, height=18, units=c("cm"), dpi=500)

spring_dev_dist_spec$strong <- gsub(" ", "\n", spring_dev_dist_spec$strong)

#Figure 3 
#strongest variable for each species group 
ggplot(data=subset(spring_dev_dist_spec, !is.na(strong)), aes(x= strong,  group=depth_profile)) + 
    geom_bar(aes(y = ..prop.., fill = factor(..x..)), stat="count") +
    geom_text(aes( label = scales::percent(..prop..),
                   y= ..prop.. ), stat= "count", hjust=1, size=3) +
    labs(y = "Percent", x = "strongest variable", lab.nb.digits=1, color="variable", orientation = "horiz", lab.size = 7, lab.hjust=-1, xlim=c(0,100)) +
  ...
  theme(axis.title = element_text(size = rel(1.5)),
        ...
        strip.text = element_text(size = rel(1))) + scale_fill_simpsons() + scale_color_simpsons() +
  theme(legend.position = "none") + 
  ggsave(filename= "Figure2_spring", plot=last_plot(), width = 18, height=9, units=c("cm"), dpi=500)
